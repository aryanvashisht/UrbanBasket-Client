...
function AddProduct() {
  ...
  const validateForm = () => {
    ...
    if (!formData.displayimage) newErrors.image = 'Product image is required';
    return newErrors;
  };
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      if (file.type.startsWith('image/') && file.size <= 5 * 1024 * 1024) { // 5MB limit
        ...
        setErrors(prev => ({ ...prev, displayimage: 'Please upload a valid image (max 5MB)' }));
      }
    }
  };
  ...
  const handleSubmit = async () => {    
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/products/addProduct`, formData, {
        headers:{
      ...
      if (error.response) {
        console.log("Error status while adding new product :: ", error.response.status);
        console.log("Error message while adding new product :: ", error.response.data.message);
      } else {
        console.log("No response from server to Add New Product");
      }
    }
